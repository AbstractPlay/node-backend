...
async function onetimeFixUpdateCompletedGames(userId: string) {
  ...
  try {
    ...
    console.log(`Got ${list.length} completed games`);

    await Promise.all(list.map(game => updateCompletedGame(game)));
  } catch (err) {
    logGetItemError(err);
    return formatReturnError('Unable to update COMPLETEDGAMES');
  }
}

async function updateCompletedGame(game0: Game) {
  const data = await ddbDocClient.send(
    new GetCommand({
      TableName: process.env.ABSTRACT_PLAY_TABLE,
      Key: {
        "pk": "GAME",
        "sk": game0.metaGame + "#1#" + game0.id
      },
    }));
  const game = data.Item as unknown as FullGame;
  const numMoves = JSON.parse(game.state).stack.length - 1;
  console.log(`Updating ${game0.metaGame} ${game0.id} ${numMoves}`)
  let work: Promise<any>[] = [];

  work.push(ddbDocClient.send(new UpdateCommand({
    TableName: process.env.ABSTRACT_PLAY_TABLE,
    Key: { "pk": "COMPLETEDGAMES", "sk": game0.sk },
    ExpressionAttributeValues: { ":n": numMoves },
    UpdateExpression: "set numMoves = :n",
  })));

  work.push(ddbDocClient.send(new UpdateCommand({
    TableName: process.env.ABSTRACT_PLAY_TABLE,
    Key: { "pk": "COMPLETEDGAMES#" + game.metaGame, "sk": game0.sk },
    ExpressionAttributeValues: { ":n": numMoves },
    UpdateExpression: "set numMoves = :n",
  })));
  ...
  game.players.forEach((player: { id: string; }) => {
    work.push(ddbDocClient.send(new UpdateCommand({
      TableName: process.env.ABSTRACT_PLAY_TABLE,
      Key: { "pk": "COMPLETEDGAMES#" + player.id, "sk": game0.sk },
      ExpressionAttributeValues: { ":n": numMoves },
      UpdateExpression: "set numMoves = :n",
    })));
    ...
    work.push(ddbDocClient.send(new UpdateCommand({
      TableName: process.env.ABSTRACT_PLAY_TABLE,
      Key: { "pk": "COMPLETEDGAMES#" + game.metaGame + "#" + player.id, "sk": game0.sk },
      ExpressionAttributeValues: { ":n": numMoves },
      UpdateExpression: "set numMoves = :n",
    })));
  });
