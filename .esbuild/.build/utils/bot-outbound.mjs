...
var REGION, clnt, ddbDocClient;
var init_ddb = __esm({
  "lib/ddb.ts"() {
    "use strict";
    REGION = "us-east-1";
    clnt = new DynamoDBClient({ region: REGION });
...
function decodePrivateKeyEnv(raw) {
...
function normalizePrivateKeyMaterial(raw) {
...
function loadPrivateKey(keyMaterial) {
  try {
...
function signBotPayload(rawBody) {
...
var cachedKey, PEM_BLOCK_RE;
var init_botSigning = __esm({
  "lib/botSigning.ts"() {
    "use strict";
    PEM_BLOCK_RE = /-----BEGIN ([A-Z0-9 ]+)-----([\s\S]*?)-----END \1-----/;
  }
});
...
var REGION2, sqsClient;
var init_wsBroadcast = __esm({
  "lib/wsBroadcast.ts"() {
    "use strict";
    REGION2 = "us-east-1";
    sqsClient = new SQSClient({ region: REGION2 });
  }
});
...
var REGION3, cognitoClient;
var init_botSecrets = __esm({
  "lib/botSecrets.ts"() {
    "use strict";
    REGION3 = "us-east-1";
    cognitoClient = new CognitoIdentityProviderClient({ region: REGION3 });
  }
});
...
// lib/botNames.ts
import { DeleteCommand as DeleteCommand4, GetCommand as GetCommand4, PutCommand as PutCommand3 } from "@aws-sdk/lib-dynamodb";
...
// api/testBot.ts
import { GetCommand as GetCommand5, PutCommand as PutCommand4, UpdateCommand as UpdateCommand4 } from "@aws-sdk/lib-dynamodb";
...
async function humanPlayerIds(client, tableName2, players) {
  ...
  for (const player of players) {
    const bot = await client.send(new GetCommand6({
      TableName: tableName2,
...
async function loadGameForAdminDelete(client, tableName2, metaGame, gameId, preferredCbit) {
  ...
  for (const cbit of order) {
    const data = await client.send(new GetCommand6({
      TableName: tableName2,
...
function createProbeEngine(gameEngine, metaGame) {
  ...
  if (metaGame && typeof engine.cheapSerialize === "function") {
    return GameFactory4(metaGame, engine.cheapSerialize());
  }
...
function engineAtMove(metaGame, gameState, move) {
  const engine = GameFactory4(metaGame, gameState);
  if (!engine) {
...
// lib/soloGame.ts
import { gameinfo, GameFactory as GameFactory5 } from "@abstractplay/gameslib";
  ...
  "lib/soloGame.ts"() {
    ...
    createSoloEngine = (metaGame, variants, challengeSeed) => {
      ...
      if (info.playercounts.length > 1) {
        engine = GameFactory5(metaGame, 1, variants);
      } else {
        engine = GameFactory5(metaGame, void 0, variants);
      }
      if (engine === void 0) {
...
// lib/gameTimeout.ts
import { UpdateCommand as UpdateCommand5 } from "@aws-sdk/lib-dynamodb";
...
async function checkAndProcessGameTimeout(game2, deps) {
  ...
  if (Array.isArray(game2.toMove)) {
    ...
    try {
      await deps.client.send(new UpdateCommand5({
...
async function acquireDashboardMaintenanceLock(client, tableName2, userId, now = Date.now()) {
  ...
  try {
    await client.send(new UpdateCommand6({
      TableName: tableName2,
      Key: { pk: "USER", sk: userId },
...
import { DeleteCommand as DeleteCommand6, GetCommand as GetCommand8, PutCommand as PutCommand5, QueryCommand as QueryCommand5 } from "@aws-sdk/lib-dynamodb";
...
async function getPlaygroundSave(client, tableName2, userId, id) {
  const data = await client.send(new GetCommand9({
    TableName: tableName2,
...
async function loadNotificationsForDashboard(client, tableName2, userId, options) {
  ...
  for (const item of items) {
    ...
    if (options.refreshExpiry && item.expiresAt > longTtlThresholdSec) {
      work.push(client.send(new UpdateCommand8({
        TableName: tableName2,
        Key: { pk: item.pk, sk: item.sk },
...
async function dismissNotification(client, tableName2, userId, sk) {
  const pk = notificationPk(userId);
  const existing = await client.send(new GetCommand10({
    TableName: tableName2,
    Key: { pk, sk }
  }));
  if (existing.Item === void 0) {
...
// api/abstractplay.ts
var abstractplay_exports = {};
__export(abstractplay_exports, {
  authQuery: () => authQuery,
  botQuery: () => botQuery,
  botRespondToChallenge: () => botRespondToChallenge,
  changeLanguageForPlayer: () => changeLanguageForPlayer,
  createSendEmailCommand: () => createSendEmailCommand,
  formatReturnError: () => formatReturnError,
  handleCommonErrors: () => handleCommonErrors,
  initi18n: () => initi18n,
  logGetItemError: () => logGetItemError,
  query: () => query
});
import { DynamoDBClient as DynamoDBClient2 } from "@aws-sdk/client-dynamodb";
import { DynamoDBDocumentClient as DynamoDBDocumentClient8, PutCommand as PutCommand10, GetCommand as GetCommand11, UpdateCommand as UpdateCommand9, DeleteCommand as DeleteCommand9, QueryCommand as QueryCommand9, BatchGetCommand as BatchGetCommand2 } from "@aws-sdk/lib-dynamodb";
...
import { gameinfo as gameinfo3, GameFactory as GameFactory6 } from "@abstractplay/gameslib";
  ...
  if (pars.type === "current") {
    try {
      ...
      const returnlist = gamelist.map((g) => {
        const state = GameFactory6(g.metaGame, g.state);
        if (state === void 0) {
...
${JSON.stringify(noteData)}`);
    ...
    if (game2.gameEnded === void 0) {
      const engine = GameFactory6(game2.metaGame, game2.state);
      if (engine === void 0) {
...
async function playerAbout(pars) {
  ...
  try {
    const tableName2 = process.env.ABSTRACT_PLAY_TABLE;
    const userData = await ddbDocClient2.send(new GetCommand11({
      TableName: tableName2,
    ...
    const botData = await ddbDocClient2.send(new GetCommand11({
      TableName: tableName2,
...
async function toggleStar(userid, pars) {
  try {
    ...
    const list = [];
    list.push(
      ddbDocClient2.send(new UpdateCommand9({
        TableName: process.env.ABSTRACT_PLAY_TABLE,
...
async function updateUserSettings(userid, pars) {
  try {
    await ddbDocClient2.send(new UpdateCommand9({
      TableName: process.env.ABSTRACT_PLAY_TABLE,
      Key: { "pk": "USER", "sk": userid },
...
async function updateBot(claim, pars) {
  ...
  try {
    const data = await ddbDocClient2.send(new GetCommand11({
      TableName: process.env.ABSTRACT_PLAY_TABLE,
      Key: {
        pk: "BOT",
        sk: clientId
...
async function updateUserEMail(claim) {
  if (claim.email && claim.email.trim().length > 0) {
    ...
    return ddbDocClient2.send(new UpdateCommand9({
      TableName: process.env.ABSTRACT_PLAY_TABLE,
      Key: { "pk": "USER", "sk": claim.sub },
...
async function newSetting(userId, pars) {
  ...
  const work = [];
  work.push(ddbDocClient2.send(new UpdateCommand9({
    TableName: process.env.ABSTRACT_PLAY_TABLE,
    Key: { "pk": "USER", "sk": userId },
    ExpressionAttributeValues: { ":v": val },
...
async function newChallenge(userid, challenge) {
  ...
  const updateChallenger = ddbDocClient2.send(new UpdateCommand9({
    TableName: process.env.ABSTRACT_PLAY_TABLE,
    Key: { "pk": "USER", "sk": userid },
...
async function removeAChallenge(challenge, standing, revoked, started, quitter) {
  ...
  if (!standing) {
    list.push(sendCommandWithRetry(new UpdateCommand9({
      TableName: process.env.ABSTRACT_PLAY_TABLE,
...
async function acceptChallenge(userid, metaGame, challengeId, standing) {
  ...
  if ((players ? players.length : 0) === challenge.numPlayers - 1) {
    ...
    let engine;
    if (info.playercounts.length > 1)
      engine = GameFactory6(challenge.metaGame, challenge.numPlayers, variants);
    else
      engine = GameFactory6(challenge.metaGame, void 0, variants);
    if (!engine)
    ...
    if (!await isBotId(userid)) {
      userUpdates.push(sendCommandWithRetry(new UpdateCommand9({
        TableName: process.env.ABSTRACT_PLAY_TABLE,
        Key: { "pk": "USER", "sk": userid },
        UpdateExpression: "DELETE challenges_received :c",
...
async function submitMove(userid, pars) {
  ...
  try {
    ...
    if (game2.gameEnded === void 0) {
      const engine2 = GameFactory6(game2.metaGame, game2.state);
      if (engine2 === void 0) {
...
async function tournamentUpdates(game2, players, timeout2) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    work.push(sendCommandWithRetry(new UpdateCommand9({
      TableName: process.env.ABSTRACT_PLAY_TABLE,
  ...
  const tournamentData = await sendCommandWithRetry(new UpdateCommand9({
    TableName: process.env.ABSTRACT_PLAY_TABLE,
...
async function saveExploration(userid, pars) {
  ...
  let gameVariants2;
  try {
    const gameData = await ddbDocClient2.send(new GetCommand11({
      TableName: process.env.ABSTRACT_PLAY_TABLE,
      Key: {
        pk: "GAME",
...
async function botMove(pars) {
  ...
  const engine = GameFactory6(pars.metaGame, game2.state);
  if (!engine)
  ...
  try {
    for (let d = 0; d < pars.divisions.length; d++) {
      ...
      for (const pid of division) {
        const cmd = ddbDocClient2.send(new UpdateCommand9({
          TableName: process.env.ABSTRACT_PLAY_TABLE,
...
${JSON.stringify(pars.pairs, null, 2)}`);
  ...
  try {
    ...
    for (const pair of pars.pairs) {
      ...
      let engine;
      if (info.playercounts.length > 1)
        engine = GameFactory6(pair.metagame, 2, pair.variants);
      else
        engine = GameFactory6(pair.metagame, void 0, pair.variants);
      if (!engine) {
...
async function eventUpdates(pars) {
  const work = [];
  work.push(
    sendCommandWithRetry(new UpdateCommand9({
      TableName: process.env.ABSTRACT_PLAY_TABLE,
      Key: { "pk": "ORGEVENTGAME", "sk": `${pars.eventid}#${pars.gameid}` },
...
async function realPingBot(metaGame, gameid, game2) {
  ...
  const engine = GameFactory6(metaGame, game2.state);
  if (!engine)
    throw new Error(`Unknown metaGame ${metaGame}`);
  const info = gameinfo3.get(metaGame);
  if (!engine.gameover) {
    ...
    if (ids.includes(process.env.AIAI_USERID)) {
      ...
      const cmd = new SendMessageCommand2(input);
      await sqsClient2.send(cmd);
    }
  }
...
var import_web_push2, import_totp, LOCALE_RESOURCES, REGISTERED_LANGUAGES, REGION4, sesClient, sqsClient2, cognitoClient2, clnt2, marshallOptions, unmarshallOptions, translateConfig, ddbDocClient2, headers, sleep, DEFAULT_META_GAME_COUNTS, query, botQuery, authQuery, getAllUsers;
var init_abstractplay = __esm({
  "api/abstractplay.ts"() {
    ...
    REGION4 = "us-east-1";
    sesClient = new SESClient({ region: REGION4 });
    sqsClient2 = new SQSClient2({ region: REGION4 });
    cognitoClient2 = new CognitoIdentityProviderClient2({ region: REGION4 });
    clnt2 = new DynamoDBClient2({ region: REGION4 });
    marshallOptions = {
...
// lib/botOutbound.ts
import { GetCommand as GetCommand12, UpdateCommand as UpdateCommand10 } from "@aws-sdk/lib-dynamodb";
import { SQSClient as SQSClient3, SendMessageCommand as SendMessageCommand3 } from "@aws-sdk/client-sqs";
import { GameFactory as GameFactory7 } from "@abstractplay/gameslib";
async function enqueueBotOutbound(message) {
  ...
  await sqsClient3.send(
    new SendMessageCommand3({
      QueueUrl: queueUrl,
...
function buildChallengersList(challenge, botId) {
...
function buildOutChallengePayload(challenge, botId) {
  return {
    verb: "challenge",
    metaGame: challenge.metaGame,
    variants: challenge.variants ?? [],
...
function buildOutMovePayload(game2, botId) {
  const engine = GameFactory7(game2.metaGame, game2.state);
  if (!engine) {
    throw new Error(`Unknown metaGame ${game2.metaGame}`);
  }
  const botIndex = game2.players.findIndex((p) => p.id === botId);
  if (botIndex === -1) {
    throw new Error(`Bot ${botId} is not a player in game ${game2.id}`);
  }
  const botPlayer = game2.players[botIndex];
  const moves = engine.moveHistory();
  const engineWithContext = engine;
  const context = typeof engineWithContext.botContext === "function" ? engineWithContext.botContext() : null;
  const payload = {
    verb: "move",
    metaGame: game2.metaGame,
    variants: game2.variants ?? [],
    gameid: game2.id,
    clockCurr: Math.round((botPlayer.time ?? 0) / 36e5),
    numPlayers: game2.numPlayers,
    moves
  };
...
async function recordBotContact(botId, statusCode, reachable) {
  await ddbDocClient.send(
    new UpdateCommand10({
      TableName: process.env.ABSTRACT_PLAY_TABLE,
      Key: { pk: "BOT", sk: botId },
...
async function postToBot(bot, body, method = "POST", expectedStatus = 200) {
  ...
  try {
    const response = await fetch(bot.endpoint, {
      method,
      headers: headers2,
      body: method === "POST" ? rawBody : void 0,
      signal: AbortSignal.timeout(BOT_HTTP_TIMEOUT_MS)
    });
    statusCode = response.status;
    reachable = true;
  } catch (error2) {
    console.error(`Failed to reach bot ${bot.sk} at ${bot.endpoint}:`, error2);
    statusCode = 0;
  }
...
async function loadChallengeRecord(challengeId, metaGame, standing) {
  const data = await ddbDocClient.send(
    new GetCommand12({
      TableName: process.env.ABSTRACT_PLAY_TABLE,
      Key: {
...
async function processBotChallengeMessage(message) {
  ...
  const { botRespondToChallenge: botRespondToChallenge2 } = await Promise.resolve().then(() => (init_abstractplay(), abstractplay_exports));
  await botRespondToChallenge2(
    message.botId,
    message.challengeId,
    message.metaGame,
    message.standing,
...
async function processBotMoveMessage(message) {
  const bot = await getBotRecord(message.botId);
  if (!bot) {
    console.error(`Bot ${message.botId} not found for move notification in game ${message.gameid}`);
    return;
  }
  const game2 = await loadGameRecord(message.metaGame, message.gameid);
  if (!game2) {
    console.log(`Game ${message.gameid} not found; skipping bot move notification`);
    return;
  }
  const payload = buildOutMovePayload(game2, message.botId);
  await postToBot(bot, payload, "POST", 202);
}
var REGION5, BOT_HTTP_TIMEOUT_MS, sqsClient3;
...
async function processRecord(record) {
  ...
  try {
    ...
    console.error("Invalid bot outbound SQS message JSON", record.body, error2);
    return;
  }
